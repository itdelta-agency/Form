<template>
  <main class="container mx-auto">
    ...
    <section
      id="firstStep"
      class="flex flex-col border-2 px-12 border-red-100"
    >
      <div class="steps px-6 h-80">
        ...
        <div v-if="step === 2" class="flex justify-start flex-col">
          <input-field :label="'Middle name'" v-model:value="middleName" />
        </div>
        <div v-if="step === 3" class="flex justify-start flex-col">
          <input-field :label="'Date of birth'" date v-model:value="dateOfBirth" />
        </div>
        <div v-if="step === 4">
          <toast />
      ...
      <div class="flex justify-center items-end w-full">
        <button type="button" class="btn btn-primary" v-if="step !== 1" @click="prevStep()">
          Prev
        </button>
        <button class="btn btn-primary ml-2" @click="checkForm()" v-if="step !== 4">Next</button>
      </div>
    </section>
  </main>
</template>
